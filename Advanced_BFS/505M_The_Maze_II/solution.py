  ...
  while queue.qsize()>0:
    ...
    if (r, c) == (start_r, start_c) or wall_hitted:
      ...
      for i in range(4):
        ...
        if new_r in range(R) and new_c in range(C) and maze[new_r][new_c] != 1 and (new_r, new_c, i) not in dist:
          # Encounter new state with new dist
          dist[(new_r, new_c, i)] = dist[(r, c, d)] + 1
